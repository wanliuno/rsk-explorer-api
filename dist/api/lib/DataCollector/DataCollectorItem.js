...
function formatResponse(result, pages) {
  if (!result) return;
...
async function getFieldsTypes(collection) {
  ...
  for (let p in doc) {
    ...
    type = value instanceof _mongodb.ObjectID ? _types.OBJECT_ID : type;
    fields[p] = type;
  }
  return fields;
}

function filterSort(sort, sortable, defaultSort) {
  let filteredSort = {};
...
function fieldFilterParse(field, value, query) {
  query = query || {};
